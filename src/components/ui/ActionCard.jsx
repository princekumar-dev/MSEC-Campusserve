    ...
    <Link to={to} className="premium-action group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {badge !== undefined && badge !== null && (
          <span className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[9px] font-bold ${badgeColor}`}>
            {badge}
          </span>
        )}
      </div>
      <ChevronRight size={16} className="text-slate-300 group-hover:text-violet-500 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
    </Link>
  )
}
